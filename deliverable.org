...
+ First MVP (Fall Semester): To have a small demo web app that only accepts very specific code
    types and test scripts.
  + Create a working backend
    + The baseline code for administration and logging
    + Type verification for submissions (.zip, .tar, etc)
    + Kubernetes calls to spin up jobs upon submission
    + API Service to relay output to the front end
  + Have a front end November
+ Second MVP
